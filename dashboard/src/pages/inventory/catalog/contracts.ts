...
const record = (value: unknown, message: string): Record<string, unknown> => {
  if (!value || typeof value !== "object" || Array.isArray(value)) catalogContractError(message);
  return value as Record<string, unknown>;
};
const integer = (value: unknown, field: string, min = 0): number => {
  if (typeof value !== "number" || !Number.isSafeInteger(value) || value < min || value > DB_INT_MAX) catalogContractError(`حقل ${field} في عقد الكتالوج غير صالح.`);
  return value;
};
const requiredString = (value: unknown, field: string, max: number): string => {
  if (typeof value !== "string" || !value.trim() || value.length > max) catalogContractError(`حقل ${field} في عقد الكتالوج غير صالح.`);
  return value;
};
...
const cursorPage = <T>(raw: unknown, parseItem: (value: unknown) => T): CursorPage<T> => {
  ...
  if (!Array.isArray(row.items) || row.items.length > 200 || typeof row.has_more !== "boolean") catalogContractError("صفحة الكتالوج غير صالحة.");
  const next = row.next_cursor === null ? null : requiredString(row.next_cursor, "next_cursor", MAX_CURSOR_LENGTH);
  if (row.has_more !== (next !== null)) catalogContractError("ترقيم الكتالوج غير متسق.");
...
export const parseUoms = (raw: unknown): UomRef[] => {
  ...
  if (!Array.isArray(row.items) || row.items.length > 100) catalogContractError("قائمة وحدات القياس غير صالحة.");
...
export const parseCatalogVariant = (item: unknown): CatalogVariant => {
  ...
  if (scale > 6) catalogContractError("دقة كمية SKU غير صالحة.");
  ...
  if (!["DRAFT", "ACTIVE", "RETIRING", "ARCHIVED"].includes(String(lifecycle))) catalogContractError("حالة SKU غير صالحة.");
  if (!["NONE", "SALES_HOLD", "RECALL"].includes(String(hold))) catalogContractError("حالة إيقاف SKU غير صالحة.");
  if (!["NONE", "OPTIONAL", "REQUIRED"].includes(String(lot)) || !["NONE", "OPTIONAL", "REQUIRED"].includes(String(expiry))) catalogContractError("سياسة الدفعة أو الصلاحية غير صالحة.");
  return {
...
export const parseConversions = (raw:unknown):UomConversion[] => {
  const page=record(raw,"استجابة تحويلات UOM غير صالحة."); if(!Array.isArray(page.items)||page.items.length>200)catalogContractError("تحويلات UOM غير صالحة.");
...
export const buildProductPayload = (draft: ProductDraft) => {
  ...
  if (!/^[A-Z0-9][A-Z0-9_.-]*$/.test(code) || code.length > 100) catalogContractError("كود العائلة غير صالح.");
  if (!name || name.length > 150) catalogContractError("اسم عائلة المنتج غير صالح.");
...
export const buildVariantPayload = (draft: VariantDraft) => {
  ...
  if (scale > 6) catalogContractError("دقة الكمية يجب أن تكون بين 0 و6.");
  ...
  if (!sku || sku.length > 100) catalogContractError("SKU مطلوب ولا يتجاوز 100 حرف.");
  if (!name || name.length > 200) catalogContractError("اسم SKU مطلوب ولا يتجاوز 200 حرف.");
  if (gtin && !/^(?:\d{8}|\d{12}|\d{13}|\d{14})$/.test(gtin)) catalogContractError("GTIN غير صالح.");
...
const parseLifecycle = (value: unknown): CatalogVariant["lifecycle_status"] => {
  if (!["DRAFT", "ACTIVE", "RETIRING", "ARCHIVED"].includes(String(value))) catalogContractError("حالة دورة الحياة غير صالحة.");
...
export const parseArchivePreflight = (raw: unknown): ArchivePreflight => {
  ...
  if (!Array.isArray(row.blockers) || row.blockers.length > 20 || typeof row.can_archive !== "boolean") catalogContractError("قائمة موانع الأرشفة غير صالحة.");
  return {
...
const parseProductLocation = (value: unknown): ProductLocationAssignment => {
  ...
  if (typeof flags.inbound_enabled !== "boolean" || typeof flags.outbound_enabled !== "boolean") catalogContractError("صلاحيات تشغيل الصنف غير صالحة.");
  return {
...
export const buildLifecycleCommand = (variant: CatalogVariant, reason: string) => {
  ...
  if (clean.length < 3 || clean.length > 1000) catalogContractError("سبب الإجراء مطلوب وبحد أدنى 3 أحرف.");
