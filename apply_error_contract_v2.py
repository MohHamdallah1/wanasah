...
def replace_region_once(
    ...
    if end_index <= start_index:
        fail(f"{label}: invalid anchor order")
...
for rel in TARGETS:
    ...
    if worktree_result.returncode != 0:
        fail(f"tracked target has unstaged local changes: {rel}")
    index_result = subprocess.run(
        ["git", "diff", "--cached", "--quiet", "--", str(rel)],
        cwd=ROOT,
    )
    if index_result.returncode != 0:
        fail(f"tracked target has staged local changes: {rel}")
...
middleware_start = '''class WanasahRawASGIMiddleware:
'''
...
middleware_replacement = '''class WanasahRawASGIMiddleware:
    ...
    async def __call__(self, scope, receive, send):
        ...
        try:
            ...
            raise
'''
main = replace_region_once(
    main,
    middleware_start,
    middleware_end,
    middleware_replacement,
    "main.py middleware region",
)
