...
EXPECTED_HEAD = "17b5d6328e75eb14deb0ae86d5d65c24d903a5ef"
...
def validate_constructor_surface(root: Path) -> None:
    # Count real constructor CALLS via AST, not raw text.  Raw text also matches
    # class declarations such as `class InventoryTransferHeader(Base):`.
    header_hits = []
    line_hits = []
    ...
    for path in (root / "wa_backend").rglob("*.py"):
        ...
        try:
            ...
            abort(f"تعذر تحليل {rel} أثناء فحص transfer constructors: {exc}")
        ...
        for node in ast.walk(tree):
            ...
            if isinstance(func, ast.Name):
                if func.id == "InventoryTransferHeader":
                    count_h += 1
                elif func.id == "InventoryTransferLine":
                    count_l += 1
