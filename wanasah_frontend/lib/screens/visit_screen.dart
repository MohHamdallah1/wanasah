...
class _VisitScreenState extends State<VisitScreen> {
  ...
  void _updateCartItem(int variantId, {int? cartons, int? packs}) {
    setState(() {
      ...
      if (_cartQuantities[variantId]!['cartons'] == 0 && _cartQuantities[variantId]!['packs'] == 0) {
  ...
  void _calculateLiveTotals() {
    ...
    _cartQuantities.forEach((variantId, qtyMap) {
      final variant = _productVariants.firstWhere((v) => v['id'] == variantId, orElse: () => {});
      if (variant.isNotEmpty) {
        double cartonPrice = (variant['price_per_carton'] as num?)?.toDouble() ?? 0.0;
        double packPrice = (variant['price_per_pack'] as num?)?.toDouble() ?? 0.0;
        
        tempTotal += (cartonPrice * qtyMap['cartons']!) + (packPrice * qtyMap['packs']!);
      }
    });
       ...
       if (response.statusCode == 200) {
          ...
          setState(() {
            ...
            if (quantityValue != null && quantityValue > 0 && variantId != null) {
              _cartQuantities[variantId] = {'cartons': quantityValue.toInt(), 'packs': 0};
            }
            ...
            _debtPaidController.text = (debtDouble == 0.0) ? '' : debtDouble.toStringAsFixed(2);
               ...
               : Container(
                   ...
                   child: Row(
                     children: [
                       ...
                       Expanded(child: Text('غير مصرح لك بإجراء عمليات بيع حالياً. بانتظار تفعيل خط السير من الإدارة.', style: TextStyle(color: Colors.orange[800], fontWeight: FontWeight.bold))),
                     ],
                   ),
                 ),
           // +++++++++++++++++++++++++++++++++++++
...
// --- دالة بناء الأجزاء الشرطية (الجديدة كلياً) ---
    ...
    if (_selectedOutcome == 'Sale') {
      return Form(
        ...
        child: Column(
          ...
          children: [
            Text('المنتجات المباعة (أضف الكميات):', style: Theme.of(context).textTheme.titleMedium),
            ...
            _buildNumericTextFormField(
              ...
              icon: Icons.money,
              validator: (value) {
                if (value == null || value.trim().isEmpty) return 'الرجاء إدخال الكاش المستلم';
                if (double.tryParse(value.trim()) == null) return 'الرجاء إدخال مبلغ صحيح';
                if (double.parse(value.trim()) < 0) return 'المبلغ لا يمكن أن يكون سالباً';
                return null;
              },
              onChanged: (_) {},
            ),
            ...
            _buildNumericTextFormField(
              ...
              validator: (value) {
                if (value != null && value.trim().isNotEmpty && double.tryParse(value.trim()) == null) return 'الرجاء إدخال مبلغ صحيح';
                if (value != null && value.trim().isNotEmpty && double.parse(value.trim()) < 0) return 'المبلغ لا يمكن أن يكون سالباً';
                return null;
              },
              onChanged: (_) {},
            ),
      ...
      return Column(
        ...
        children: [
          ...
          TextFormField(
            controller: _notesController,
            decoration: const InputDecoration(labelText: 'اذكر السبب أو أضف ملاحظة', border: OutlineInputBorder()),
            maxLines: 3,
          ),
          const SizedBox(height: 20),
      ...
      return Column(
        ...
        children: [
          ...
          TextFormField(
            controller: _notesController,
            decoration: const InputDecoration(labelText: 'مثال: المحل مغلق، العودة 2م', border: OutlineInputBorder()),
            maxLines: 3,
          ),
        ],
      );
  ...
  Widget _buildProductsList() {
    ...
    if (_fetchProductsError != null) return Center(child: Padding(padding: const EdgeInsets.all(8.0), child: Text('خطأ تحميل المنتجات: $_fetchProductsError', style: const TextStyle(color: Colors.red))));
    if (_productVariants.isEmpty) return const Center(child: Padding(padding: EdgeInsets.all(8.0), child: Text('لا توجد منتجات متاحة حالياً.')));

    return ListView.separated(
      shrinkWrap: true,
      physics: const NeverScrollableScrollPhysics(),
      itemCount: _productVariants.length,
      separatorBuilder: (context, index) => const SizedBox(height: 12), // فراغ أنيق بين الكروت
      itemBuilder: (context, index) {
        final variant = _productVariants[index];
        final int id = variant['id'] as int;
        final String name = variant['variant_name'] ?? 'غير معروف';
        final double cartonPrice = (variant['price_per_carton'] as num?)?.toDouble() ?? 0.0;
        final double packPrice = (variant['price_per_pack'] as num?)?.toDouble() ?? 0.0;
          ...
          child: Column(
            children: [
              ...
              Row(
                children: [
                  Container(
                    ...
                    child: Icon(Icons.inventory_2_outlined, color: Colors.blue.shade800, size: 20),
                  ),
                  const SizedBox(width: 10),
                  Expanded(
                    child: Column(
                      ...
                      children: [
                        ...
                        Text('📦 ${cartonPrice.toStringAsFixed(2)}  |  🍬 ${packPrice.toStringAsFixed(2)}', style: TextStyle(color: Colors.grey.shade700, fontSize: 13, fontWeight: FontWeight.w600)),
                      ],
                    ),
                  ),
                ],
              ...
              Row(
                ...
                children: [
                  // عداد الكراتين
                  _buildCompactCounter('كرتونة', '📦', currentCartons, 
                    () => _updateCartItem(id, cartons: currentCartons > 0 ? currentCartons - 1 : 0),
                    () => _updateCartItem(id, cartons: currentCartons + 1),
                    () => _showNumpadDialog(id, name, 'كرتونة', currentCartons, isCarton: true)
                  ),
                  Container(width: 1, height: 40, color: Colors.grey.shade300), // فاصل عمودي أنيق
                  // عداد الحبات
                  _buildCompactCounter('حبة', '🍬', currentPacks, 
                    () => _updateCartItem(id, packs: currentPacks > 0 ? currentPacks - 1 : 0),
                    () => _updateCartItem(id, packs: currentPacks + 1),
                    () => _showNumpadDialog(id, name, 'حبة', currentPacks, isCarton: false)
                  ),
                ],
              )
            ],
  ...
  Widget _buildCompactCounter(String label, String emoji, int qty, VoidCallback onMinus, VoidCallback onPlus, VoidCallback onTapNum) {
    final bool hasQty = qty > 0;
    return Column(
      children: [
        Text('$emoji $label', style: TextStyle(fontSize: 12, color: Colors.grey.shade600, fontWeight: FontWeight.bold)),
        const SizedBox(height: 4),
        Row(
          children: [
            InkWell(
              onTap: hasQty ? onMinus : null,
              child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: hasQty ? Colors.red.shade50 : Colors.grey.shade100, borderRadius: BorderRadius.circular(6)), child: Icon(Icons.remove, size: 20, color: hasQty ? Colors.red : Colors.grey.shade400)),
            ),
            GestureDetector(
              onTap: onTapNum,
              child: Container(width: 45, alignment: Alignment.center, child: Text('$qty', style: TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: hasQty ? Colors.green.shade700 : Colors.black87))),
            ),
            InkWell(
              onTap: onPlus,
              child: Container(padding: const EdgeInsets.all(4), decoration: BoxDecoration(color: Colors.green.shade50, borderRadius: BorderRadius.circular(6)), child: const Icon(Icons.add, size: 20, color: Colors.green)),
            ),
          ],
        )
      ],
  ...
  Future<void> _showNumpadDialog(int variantId, String productName, String unitName, int currentQty, {required bool isCarton}) async {
    final TextEditingController qtyController = TextEditingController(text: currentQty > 0 ? currentQty.toString() : '');
    await showDialog(
      context: context,
      builder: (context) {
        return AlertDialog(
          shape: RoundedRectangleBorder(borderRadius: BorderRadius.circular(15)),
          title: Text('إدخال عدد الـ $unitName', style: const TextStyle(fontSize: 16)),
          content: TextField(
            controller: qtyController, keyboardType: TextInputType.number, inputFormatters: [FilteringTextInputFormatter.digitsOnly],
            decoration: const InputDecoration(hintText: 'الكمية', border: OutlineInputBorder(), filled: true, fillColor: Colors.white), autofocus: true, textAlign: TextAlign.center, style: const TextStyle(fontSize: 22, fontWeight: FontWeight.bold),
          ),
          actions: [
            ...
            ElevatedButton(
              onPressed: () {
                ...
                if (newQty != null && newQty >= 0) {
                  isCarton ? _updateCartItem(variantId, cartons: newQty) : _updateCartItem(variantId, packs: newQty);
                } else if (qtyController.text.trim().isEmpty) {
                  isCarton ? _updateCartItem(variantId, cartons: 0) : _updateCartItem(variantId, packs: 0);
                }
        ...
        children: [
          Row(
            ...
            children: [
              ...
              Text('${_totalExpectedValue.toStringAsFixed(2)} د.أ', style:  TextStyle(fontWeight: FontWeight.bold, fontSize: 20, color: Colors.blue[800])),
            ],
          ),
          const Divider(),
          Row(
            mainAxisAlignment: MainAxisAlignment.spaceBetween,
            children: [
              const Text('البونص المستحق:', style: TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
              Text('$_totalBonusItems حبة (قريباً)', style: const TextStyle(color: Colors.green, fontWeight: FontWeight.bold)),
            ],
          ),
        ],
      ),
...
// --- دالة التحقق والإرسال الذكية ---
  Future<void> _validateAndSubmit() async {
    ...
    if (_selectedOutcome == 'Sale') {
      ...
      if (_cartQuantities.isEmpty || _cartQuantities.values.every((qtyMap) => qtyMap['cartons']! <= 0 && qtyMap['packs']! <= 0)) {
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('الرجاء إضافة كمية لمنتج واحد على الأقل.'), backgroundColor: Colors.orange));
        return;
      }
      ...
      if (cashEntered > _totalExpectedValue) {
        ScaffoldMessenger.of(context).showSnackBar(SnackBar(content: Text('خطأ: الكاش (${cashEntered.toStringAsFixed(2)}) أكبر من قيمة البضاعة (${_totalExpectedValue.toStringAsFixed(2)}).'), backgroundColor: Colors.orange));
        return;
      ...
      await _performSubmit(cashEntered, debtPaidEntered, _notesController.text.trim().isNotEmpty ? _notesController.text.trim() : null);
    
    } else if (_selectedOutcome == 'NoSale') {
      final double debtPaidEntered = double.tryParse(_debtPaidController.text.trim()) ?? 0.0;
      final String? notesOrReason = _notesController.text.trim().isNotEmpty ? _notesController.text.trim() : null;
      if (debtPaidEntered < 0) { 
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('مبلغ تحصيل الذمة لا يمكن أن يكون سالباً.'), backgroundColor: Colors.orange)); 
        return; 
      }
      ...
      if (notesOrReason == null || notesOrReason.isEmpty) { 
        ScaffoldMessenger.of(context).showSnackBar(const SnackBar(content: Text('الرجاء إدخال سبب التأجيل أو ملاحظة للمتابعة.'), backgroundColor: Colors.orange));
        return; 
      }
  ...
  Future<void> _performSubmit(double cashCollected, double debtPaid, String? notes) async {
    ...
    try {
      Map<String, String> headers = await getAuthenticatedHeaders(needsContentType: false);
      ...
      if (response.statusCode == 200) {
        ...
        try { 
          ...
          if (errorData is Map && errorData.containsKey('message')) errorMessage = errorData['message'];
        } catch (_) {}
