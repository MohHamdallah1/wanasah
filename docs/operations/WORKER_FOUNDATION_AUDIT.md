...
- [x] **Periodic recovery:** operational recovery runs every 10 minutes; product-import recovery runs every 5 minutes while workers are alive.
- [x] **Worker-process failure:** a stalled safe operational job is returned to `todo`; a non-allowlisted job is left fail-closed.
...
- [x] **Reports safety:** report work remains tenant-scoped and PostgreSQL `READ ONLY`, with dedicated startup recovery.
...
- [x] **Scheduling:** stale handshakes, stale sessions, integrity, Live Stock transitions, stalled recovery, cleanup, and product-import recovery retain explicit periodic schedules.
