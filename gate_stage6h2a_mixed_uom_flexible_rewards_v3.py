...
def read(rel: str) -> str:
    path = ROOT / rel
    if not path.is_file():
        # Missing files are already reported explicitly by static_checks().
        # Return an empty sentinel so the gate reports all related failures
        # instead of crashing with FileNotFoundError.
        return ""
    return path.read_text(encoding="utf-8")


def has_component_key_current_annotation(source: str) -> bool:
    """Verify the engine current-net map is typed per (line_id, uom_id).

    This is intentionally AST-based so code formatting cannot create a false
    failure. It is paired with the runtime stacking/clamping test later in the
    same gate, so this does not weaken behavioral coverage.
    """
    try:
    ...
    for node in ast.walk(tree):
        if not isinstance(node, ast.AnnAssign):
            continue
        if not isinstance(node.target, ast.Name) or node.target.id != "current":
            continue
        ...
        if (
            ...
            and first.id == "ComponentKey"
            and isinstance(second, ast.Name)
            and second.id == "Decimal"
        ):
            return True
...
def static_checks() -> None:
    ...
    check(
        has_component_key_current_annotation(engine)
        and "adjustment.uom_id" in engine
        and "current_key" in engine,
        "Discount stacking/clamping is structurally keyed per UOM component",
    )
    check(
