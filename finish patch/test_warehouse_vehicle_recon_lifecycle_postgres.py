...
async def ensure_schema_ready() -> None:
    async with AsyncSessionLocal() as db:
            ...
            await db.execute(
                text(
                    ...
                    WHERE ns.nspname = current_schema()
                      AND (
                            (
                                cls.relname = 'work_sessions'
                                AND con.conname =
                                    'ck_work_sessions_chk_work_session_settlement_requires_end'
                            )
                            OR
                            (
                                cls.relname = 'stocktake_sessions'
                                AND con.conname =
                                    'ck_stocktake_sessions_chk_stocktake_work_session_scope'
                            )
                      )
                    """
                )
        ...
        checks = {
            ...
            for table_name, constraint_name, definition in check_rows
        }
        ...
        if set(checks) != expected_checks:
            raise AssertionError(
                f"missing lifecycle CHECK constraints: {checks}"
            )

        work_session_check = checks[
            (
                "work_sessions",
                "ck_work_sessions_chk_work_session_settlement_requires_end",
            )
        ...
        if "IS_SETTLED" not in work_session_check or "END_TIME" not in work_session_check:
            raise AssertionError(
                f"bad WorkSession lifecycle CHECK: {work_session_check}"
            )

        stocktake_check = checks[
            (
                "stocktake_sessions",
                "ck_stocktake_sessions_chk_stocktake_work_session_scope",
            )
        ...
        for token in (
            ...
            if token not in stocktake_check:
                raise AssertionError(
                    f"bad VEHICLE_RECON lifecycle CHECK: {stocktake_check}"
                )
