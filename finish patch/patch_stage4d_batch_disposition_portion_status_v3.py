...
def find_generated_migration(root: Path) -> Path:
    ...
    for path in sorted(versions.glob("*.py")):
        try:
            ...
            continue
        try:
            _revision, down_revision = parse_revision(text)
        except SystemExit:
            continue
        if (
            down_revision == EXPECTED_DOWN_REVISION
            and "def upgrade" in text
            and "def downgrade" in text
        ):
            matches.append(path)
    ...
    if len(matches) != 1:
        ...
        abort(
            "تعذر تحديد Migration Stage 4D المولدة بشكل وحيد بناءً على "
            f"down_revision={EXPECTED_DOWN_REVISION}. found={names}"
        )
    return matches[0]
