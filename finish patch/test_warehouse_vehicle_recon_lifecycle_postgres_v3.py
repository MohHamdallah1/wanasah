...
async def ensure_schema_ready() -> None:
    async with AsyncSessionLocal() as db:
        ...
        normalized = [
            ...
            for table_name, constraint_name, definition in constraint_rows
        ]

        # Do not depend on exact PostgreSQL constraint names. SQLAlchemy's
        # naming convention prefixes ck_<table>_, and PostgreSQL identifiers
        # can also be truncated. Verify the actual CHECK semantics instead.
        settlement_check = any(
            table_name == "work_sessions"
            and "is_settled is false" in definition
            and "end_time is not null" in definition
            for table_name, _name, definition in normalized
        )
        work_session_scope_check = any(
            table_name == "stocktake_sessions"
            and "stocktake_type = 'vehicle_recon'" in definition
            and "related_work_session_id is not null" in definition
            and "stocktake_type <> 'vehicle_recon'" in definition
            and "related_work_session_id is null" in definition
            for table_name, _name, definition in normalized
        )

        if not settlement_check or not work_session_scope_check:
            raise AssertionError(
                "missing lifecycle CHECK semantics: "
                f"settlement={settlement_check}, "
                f"work_session_scope={work_session_scope_check}, "
                f"actual={normalized}"
            )
...
async def main() -> None:
    ...
    for company_id in (fx["c1"], fx["c2"]):
        try:
            ...
            cleanup_errors.append(f"company {company_id}: {exc}")

    try:
        await engine.dispose()
    except Exception as exc:
        cleanup_errors.append(f"engine dispose: {exc}")
    ...
    if cleanup_errors:
        raise RuntimeError(
            "VEHICLE_RECON_TEST_CLEANUP_FAILED: "
            + " | ".join(cleanup_errors)
        )
