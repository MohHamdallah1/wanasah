...
COMPONENT_TESTS = [
    ...
    (
        "test_warehouse_vehicle_recon_lifecycle_postgres.py",
    ...
    (
        "test_warehouse_freeze_supplement_postgres.py",
...
def static_audit() -> None:
    ...
    if len(routes) != 20:
        fail(f"Warehouse route count drifted: {len(routes)} != 20")
    ...
    # No route-level ad-hoc InventoryBalance mutation.
    ...
    # Heuristic defense-in-depth: every tenant-model select window in warehouse/services
    # must contain company_id. RLS remains the second independent layer.
    tenant_models = (
    ...
    for label, text_value in (("warehouse.py", warehouse_text), ("services.py", services_text)):
        for match in re.finditer(r"select\(([^\n\)]{0,160})", text_value):
            ...
            if "company_id" not in window:
                ...
                fail(f"Possible tenant-unscoped SELECT in {label}:{line}: {head.strip()}")
    ...
    if warehouse_text.count("request_id") < 4:
        fail("Request-level idempotency contract appears incomplete")

    if "date.today(" in warehouse_text or "date.today(" in services_text:
        fail("Business inventory decisions must not use process-local date.today()")

    # Verify model source still has exactly 50 mapped table declarations without importing DB config.
    table_names = []
    ...
    print("ROUTES=20_ALL_ADMIN_GUARDED")
...
def run_component(script_name: str, success_marker: str) -> None:
    script = ROOT / script_name
    if not script.is_file():
        fail(
            f"Missing {script_name}. Keep the previously used PostgreSQL test files "
            "in the project root before running the freeze master."
        )
    ...
    if proc.returncode != 0:
        fail(f"{script_name} failed with exit code {proc.returncode}")
    if success_marker not in proc.stdout:
        fail(f"{script_name} exited 0 but missing success marker {success_marker}")


def main() -> None:
    static_audit()
    for script_name, marker in COMPONENT_TESTS:
        run_component(script_name, marker)

    print("\nWAREHOUSE_FREEZE_MASTER_POSTGRES_OK")
    print("TENANT_ISOLATION=VERIFIED_AT_API_AND_RLS_LAYERS")
