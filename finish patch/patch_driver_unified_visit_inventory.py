...
PATCH_ID = "DRIVER_UNIFIED_VISIT_INVENTORY"
ROOT = Path.cwd()
...
# Zero-Trust / Smart Automation contract: sales never accept batch_id from the client.
s, e, b = class_block(schemas, "VisitItemInput(RequestModel):", "class VisitReturnInput")
if re.search(r"^\s+batch_id\s*:", b, flags=re.M):
    raise SystemExit(
        "PATCH_ABORT: VisitItemInput يحتوي batch_id؛ المبيعات يجب أن تبقى Auto-FEFO من السيرفر فقط."
    )
...
# Normalize the models import surface here; do not depend on a previous import edit.
def normalize_models_import(text: str) -> str:
    desired = [
        "Driver", "WorkSession", "DispatchRoute", "Visit", "WorkBreakLog",
        "VisitItem", "Shop", "ProductVariant", "VisitReturn", "OfferRule", "Zone",
        "ShortageRequest", "SystemAuditLog", "InventoryLocation", "InventoryBalance",
        "InventoryMovement", "ProductBatch", "InventoryTransferHeader",
        "InventoryTransferLine", "SessionInventorySnapshot", "InventoryDamageEvent",
    ]
    legacy = {
        "InventoryTransfer", "MainWarehouse", "WarehouseLedger",
        "SessionInventory", "InventoryLedger", "VehicleLoad",
    }

    multi = re.search(
        r"^from models import \((?P<body>.*?)\)\s*$",
        text,
        flags=re.M | re.S,
    )
    if multi:
        raw_names = re.findall(r"\b[A-Za-z_][A-Za-z0-9_]*\b", multi.group("body"))
        names = []
        for name in raw_names:
            if name in legacy or name in names:
                continue
            names.append(name)
        for name in desired:
            if name not in names:
                names.append(name)
        block = "from models import (\n" + "".join(f"    {name},\n" for name in names) + ")"
        return text[:multi.start()] + block + text[multi.end():]

    single = re.search(r"^from models import (?P<body>[^\n]+)$", text, flags=re.M)
    if not single:
        raise SystemExit("PATCH_ABORT: import from models غير موجود في driver.py")
    names = [part.strip() for part in single.group("body").split(",") if part.strip()]
...
driver = normalize_models_import(driver)
...
print("SCHEMAS_OK: Sale has no batch_id; Return batch_id is required; visit request_id is required")
print("DRIVER_OK: update_visit -> Auto-FEFO sales/exchange + scanned return batch + InventoryMovement + request idempotency")
